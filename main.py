...
def start_api_server(config: dict, db_config: dict, metrics_store, logger):
    """
    Inicia servidor FastAPI em thread separada.

    Args:
        config: Configuração completa
        db_config: Configuração de databases
    ...
    try:
        ...
        init_dependencies(metrics_store, db_config, config)
...
def main():
    ...
    try:
        ...
        # IMPORTANTE: usar monitor.config (do DuckDB) ao invés do config.json
        api_thread = start_api_server(
            config=monitor.config,
            db_config=monitor.db_config,
        ...
        # IMPORTANTE: usar monitor.config (do DuckDB) ao invés do config.json
        teams_notifier = None
        if monitor.config.get('teams', {}).get('enabled', False):
            try:
                from sql_monitor.utils.teams_notifier import TeamsNotifier
                teams_notifier = TeamsNotifier(monitor.config)
