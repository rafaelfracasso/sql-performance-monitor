...
def test_factory_hana():
    """Testa factory com SAP HANA."""
    print("\n" + "=" * 80)
    print("TESTE DATABASE FACTORY - SAP HANA")
    print("=" * 80)
    ...
    try:
        print("\nCriando componentes SAP HANA via Factory...")
        conn, collector, extractor = DatabaseFactory.create_components(
            DatabaseType.HANA,
            credentials
        )
        ...
        # Testar conexão (se credenciais disponíveis)
        if all(credentials.values()):
            if conn.connect():
                print("\n   Testando conexão...")
                if conn.test_connection():
                    print("   ✓ SAP HANA Factory funcionando!")
                conn.disconnect()
                return True
            else:
                print("   ✗ Falha ao conectar")
            ...
            print("\n   ✓ SAP HANA componentes criados (conexão não testada - sem credenciais)")
            return True
...
if __name__ == '__main__':
    ...
    # Teste 4: HANA (componentes implementados)
    results.append(("HANA Factory", test_factory_hana()))
